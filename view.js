...
window.onload = function init() {
    ...
    tangentProgram = initShaders( gl, "color.vs", "color.fs" );
    ...
    let zoomController = gui.add(menu, "zoom", 1, 10);    
    zoomController.onChange(function(value) {
        setZoom();
    });

    let perspectiveController = gui.add(menu, "perspective", false);
    perspectiveController.onChange(function (value) {
        if (value)
            setPerspective();
        else
            setOrthographic();
    ...
    gui.add(menu, "shading", {'Texture Map': 0, 'Normal Map': 1, 'Height Map': 2});

    gui.add(menu, "depthScale", -10, 10);
    
    gui.add(menu, "tangentBases", false);
    ...
    */
    ...
    function setZoom () {
        ...
        proj2clip = Mat.scale(new PV(menu.zoom / aspect, menu.zoom, 1, true));
        clip2proj = Mat.scale(new PV(aspect / menu.zoom, 1 / menu.zoom, 1, true));
    ...
    (function render() {
        ...
        gl.uniform1f(gl.getUniformLocation(program, "depthScale"), menu.depthScale);
   
        var model2clipLoc = gl.getUniformLocation( program, "model2clip" );
        gl.uniformMatrix4fv(model2clipLoc, false, model2clip.flatten());
    
        sphere.render(gl, program, menu.shading);

        if(menu.tangentBases) {

          gl.useProgram(tangentProgram);

          var mvpLoc = gl.getUniformLocation( tangentProgram, "MVP");
          gl.uniformMatrix4fv(mvpLoc, false, model2clip.flatten());

          sphere.renderTangentBases(gl, tangentProgram);
        }
